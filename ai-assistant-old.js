// ============================================
// KI-SALES ASSISTANT fÃ¼r MASESites
// ============================================

// OpenAI API Configuration
const OPENAI_API_KEY = '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'; // <-- Hier deinen Key einfÃ¼gen
...
class MASEAssistant {
  ...
  createChatWidget() {
    ...
    const chatHTML = `
      <div class="ai-chat-widget" id="ai-chat-widget">
        <button class="ai-chat-toggle" id="ai-chat-toggle" aria-label="KI-Assistent Ã¶ffnen">
        ...
        <div class="ai-chat-tooltip">
          Frag mich etwas Ã¼ber unsere Leistungen
        </div>

        <div class="ai-welcome-bubble" id="ai-welcome-bubble">
          <button class="ai-welcome-bubble-close" id="ai-welcome-bubble-close" aria-label="SchlieÃŸen">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
        ...
        <div class="ai-chat-container" id="ai-chat-container">
          <div class="ai-chat-header">
            <div class="ai-chat-header-top">
              ...
              <button class="ai-chat-close" id="ai-chat-close" aria-label="Chat schlieÃŸen">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                  <line x1="18" y1="6" x2="6" y2="18"></line>
                  <line x1="6" y1="6" x2="18" y2="18"></line>
                </svg>
  ...
  toggleChat() {
    ...
    if (isOpen) {
      // Hide welcome bubble wenn Chat geÃ¶ffnet wird
      this.hideWelcomeBubble();
    ...
    const infoBoxHTML = `
      <div class="ai-info-box">
        ...
        <ul>
          ...
          <li>Produkte oder Services erklÃ¤ren</li>
        </ul>
  ...
  showLiveDemo() {
    ...
    setTimeout(() => {
      ...
      this.sendBotMessage(`<strong>Ein KI-Assistent fÃ¼r deine Website:</strong><br><br>âœ“ Beantwortet Kundenfragen 24/7<br>âœ“ Sammelt Leads automatisch<br>âœ“ Steigert deine Abschlussrate<br>âœ“ Entlastet deinen Support<br><br>Das ist nicht nur ein Chat.<br>Das ist ein <strong>Verkaufs-Tool</strong>.`);
      ...
      setTimeout(() => {
        this.showQuickReplies([
          ...
          { text: 'Wie lÃ¤uft ein Projekt ab?', action: () => this.handleProcessInquiry() }
        ]);
      }, 500);
    }, 3200);
  }
  ...
  async sendMessage() {
    ...
    try {
      ...
      this.hideTypingIndicator();
      this.sendBotMessage('Entschuldigung, technisches Problem. Schreib uns direkt: info@masesites.ch');
    }
  }
  ...
  processUserMessage(message) {
    ...
    // Kontakt / Meeting
    if (lowerMessage.includes('termin') || lowerMessage.includes('call') || lowerMessage.includes('gesprÃ¤ch')) {
  ...
  getSystemPrompt() {
    return `ROLLE
Du bist der Website- & KI-Sales Assistant von â€žMASESites AG" (Matteo & Severin).
Du schreibst auf Deutsch (Schweiz), kurz, klar, aktiv, ohne Emojis.

MISSION
- Verwandle jede Anfrage in eine qualifizierte Projektanfrage.
- Verkaufe MASESites Ã¼berzeugend, aber immer wahrheitsgemÃ¤ss.
- Keine erfundenen Referenzen, Zahlen, Kundenlogos, Awards oder Garantien.
- Wenn dir Infos fehlen: frage kurz nach oder formuliere als Option (â€žtypisch", â€žje nach Bedarf").
...
3) Optional: KI-Assistent in die Website (Lead-Qualifizierung, FAQ, Termin/Anfrage, Produkt-/Service-ErklÃ¤rung)

PREISE (Schweiz, CHF - immer als Orientierung)
- Landingpage (1 Seite): CHF 1'800 â€“ 2'900
- Business Website (3-5 Seiten): CHF 3'200 â€“ 5'400
- Premium Website (6-10 Seiten): CHF 5'900 â€“ 7'900
- KI-Assistent (Add-on): + CHF 900 â€“ 1'900 (je nach KomplexitÃ¤t)
Final nach kurzem Call (15 Min).

STANDARD-ARGUMENTE (nur wenn passend)
- Mobile-first, schnell, sauberer Code, klarer Aufbau.
- Starkes UI/UX: klare CTA, bessere NutzerfÃ¼hrung.
...
REGELN FÃœR ANTWORTEN
1) Erst klÃ¤ren, was der User will (max. 3 Fragen).
2) Dann sofort eine Empfehlung geben (konkret: Seitenstruktur + Features).
3) Immer mit einem CTA enden: â€žSoll ich dir ein Angebot erstellen?" oder â€žWillst du einen 15-min Call?"
4) Wenn der User nur â€žPreis?" fragt: Gib Preisspannen als Orientierung und sag klar: final nach kurzem Call.
5) Wenn der User uns mit anderen vergleicht: Hebe Prozess, QualitÃ¤t, Speed, KI-Integration hervor. Keine Beleidigungen anderer.
6) Wenn der User nach Dingen fragt, die wir nicht anbieten: Sag es ehrlich. Biete Alternative oder PartnerlÃ¶sung an.

LEAD-QUALIFIZIERUNG (kurz, effektiv)
Stelle je nach Kontext 2â€“3 Fragen:
- Branche + Ziel (Leads, Bewerbungen, Shop, Branding)?
- Umfang (1 Landingpage oder mehrere Seiten)?
- Deadline und Budget-Rahmen?
- KI-Assistent ja/nein? Welche Aufgaben soll er Ã¼bernehmen?

OUTPUT-FORMAT (meistens)
- 3â€“6 Bulletpoints Nutzen/Plan
- 1 kurzer Vorschlag fÃ¼r nÃ¤chste Schritte
- 1 klare Abschlussfrage (CTA)

BEISPIEL-CTA
- â€žWillst du, dass wir dir eine 1-Seiten-Struktur + grobe Offerte in CHF machen?"
- â€žSoll der KI-Assistent eher FAQ lÃ¶sen oder Leads vorqualifizieren?"

WICHTIG
- Nutze HTML-Formatting: <br> fÃ¼r ZeilenumbrÃ¼che, <strong> fÃ¼r Fettdruck
- Halte Antworten unter 150 WÃ¶rtern
- Sei direkt und handlungsorientiert`;
  ...
  getFallbackResponse(userMessage) {
    ...
    // Fallback zu rule-based responses wenn API fehlschlÃ¤gt
  ...
  getGreetingMessage() {
    return `Hallo! Ich bin dein MASESites Assistant.<br><br>Ich helfe dir, die passende Website-LÃ¶sung zu finden â€“ inkl. Struktur, Features und grobe PreiseinschÃ¤tzung.<br><br>Was mÃ¶chtest du wissen?`;
  }
  ...
  startQualification() {
    ...
    this.sendBotMessage(`Perfekt! Damit ich dir die beste LÃ¶sung empfehlen kann, brauche ich kurz 3 Infos:`);

    setTimeout(() => {
      this.sendBotMessage(`1ï¸âƒ£ <strong>Branche & Ziel:</strong><br>WofÃ¼r brauchst du die Website? (z.B. Leads generieren, Produkte verkaufen, Bewerbungen sammeln, Branding)`);
      this.showQuickReplies([
        ...
        { text: 'Branding / PrÃ¤senz', action: () => this.setLeadData('ziel', 'Branding') }
      ]);
    }, 1000);
  }
  ...
  setLeadData(key, value) {
    ...
    if (key === 'ziel') {
      setTimeout(() => {
        this.sendBotMessage(`2ï¸âƒ£ <strong>Umfang:</strong><br>Wie viele Seiten brauchst du ca.?`);
        this.showQuickReplies([
    ...
    if (key === 'umfang') {
      setTimeout(() => {
        this.sendBotMessage(`3ï¸âƒ£ <strong>KI-Assistent:</strong><br>Soll ein KI-Assistent integriert werden?`);
        this.showQuickReplies([
          { text: 'Ja, fÃ¼r FAQ & Leads', action: () => this.setLeadData('kiAssistant', 'Ja (FAQ + Leads)') },
          { text: 'Ja, nur fÃ¼r FAQ', action: () => this.setLeadData('kiAssistant', 'Ja (nur FAQ)') },
          { text: 'Nein, nicht nÃ¶tig', action: () => this.setLeadData('kiAssistant', 'Nein') }
        ]);
      }, 1000);
    }
  ...
  makeRecommendation() {
    ...
    if (this.leadData.umfang === '1 Seite') {
      recommendation += `ðŸ“„ <strong>Landingpage (1 Seite)</strong><br>`;
      recommendation += `â€¢ Klare Struktur: Hero, Leistungen, Social Proof, CTA<br>`;
      recommendation += `â€¢ Mobile-first, schnelle Ladezeit<br>`;
      recommendation += `â€¢ Kontaktformular mit Validierung<br>`;
      if (this.leadData.kiAssistant !== 'Nein') {
        recommendation += `â€¢ KI-Assistent fÃ¼r FAQ + Lead-Qualifizierung<br>`;
      }
      recommendation += `<br><strong>Preis:</strong> ca. CHF 1'800 â€“ 2'900`;
    } else if (this.leadData.umfang === '3-5 Seiten') {
      recommendation += `ðŸ“± <strong>Business Website (3-5 Seiten)</strong><br>`;
      recommendation += `â€¢ Home, Ãœber uns, Leistungen, Referenzen, Kontakt<br>`;
      recommendation += `â€¢ Professionelles Design + UX<br>`;
      recommendation += `â€¢ SEO-Grundlagen + Performance<br>`;
      if (this.leadData.kiAssistant !== 'Nein') {
        recommendation += `â€¢ KI-Assistent 24/7 verfÃ¼gbar<br>`;
      }
      recommendation += `<br><strong>Preis:</strong> ca. CHF 3'200 â€“ 5'400`;
    } else {
      recommendation += `ðŸš€ <strong>Umfangreiche Website (${this.leadData.umfang})</strong><br>`;
      recommendation += `â€¢ Individuelle Struktur nach Bedarf<br>`;
      recommendation += `â€¢ Content-Management-System (CMS)<br>`;
      recommendation += `â€¢ Erweiterte Funktionen mÃ¶glich<br>`;
      if (this.leadData.kiAssistant !== 'Nein') {
        recommendation += `â€¢ KI-Assistent mit Custom Training<br>`;
      }
      recommendation += `<br><strong>Preis:</strong> ca. CHF 5'900 â€“ 7'900+`;
    }

    recommendation += `<br><br><strong>NÃ¤chster Schritt:</strong><br>`;
  ...
  handlePriceInquiry() {
    const priceInfo = `<strong>PreisÃ¼bersicht MASESites:</strong><br><br>

ðŸ“„ <strong>Landingpage (1 Seite)</strong><br>
CHF 1'800 â€“ 2'900<br>
<small>Inkl. Design, Development, Hosting-Setup</small><br><br>

ðŸ“± <strong>Business Website (3-5 Seiten)</strong><br>
CHF 3'200 â€“ 5'400<br>
<small>Inkl. CMS, SEO-Basics, Performance</small><br><br>

ðŸš€ <strong>Premium Website (6-10 Seiten)</strong><br>
CHF 5'900 â€“ 7'900<br>
<small>Inkl. Custom Features, erweiterte Funktionen</small><br><br>

ðŸ¤– <strong>KI-Assistent (Add-on)</strong><br>
+ CHF 900 â€“ 1'900<br>
<small>Je nach KomplexitÃ¤t & Training</small><br><br>
...
âœ… Professionelles Design (Mobile-first)<br>
âœ… Sauberer Code (HTML/CSS/JS)<br>
âœ… Performance-Optimierung<br>
âœ… SEO-Grundlagen<br>
âœ… Kontaktformular<br>
âœ… DSGVO-konform (Cookie-Banner)<br>
âœ… Hosting-Setup UnterstÃ¼tzung<br>
âœ… 2-3 Feedback-Runden<br><br>

Optional: KI-Assistent, CMS, Shop-Funktion, Analytics`);
  }

  handleAIInquiry() {
    this.sendBotMessage(`<strong>KI-Assistent fÃ¼r deine Website:</strong><br><br>

ðŸ¤– <strong>Was er kann:</strong><br>
â€¢ Beantwortet FAQ 24/7<br>
â€¢ Qualifiziert Leads automatisch<br>
â€¢ Sammelt Kontaktdaten<br>
â€¢ Produktberatung / Service-ErklÃ¤rung<br>
â€¢ Terminvereinbarung<br><br>

ðŸ’° <strong>Preis:</strong> CHF 900 â€“ 1'900<br>
<small>(Je nach Umfang & Custom Training)</small><br><br>

âš¡ <strong>Nutzen:</strong><br>
Mehr Leads, weniger Support-Aufwand, hÃ¶here Conversion<br><br>
...
Willst du den KI-Assistenten in deine Website integrieren?`);
    ...
    setTimeout(() => {
      this.showQuickReplies([
        ...
        { text: 'Noch unsicher', action: () => this.sendBotMessage('Kein Problem! Wir kÃ¶nnen den KI-Assistenten auch spÃ¤ter noch hinzufÃ¼gen. Soll ich dir erst ein Website-Angebot machen?') }
      ]);
    }, 1500);
  }
...
Wir zeigen dort bewusst nur echte Cases â€“ keine Mock-ups.<br><br>

<strong>Typische Branchen:</strong><br>
â€¢ B2B-Dienstleister<br>
â€¢ Beratung & Coaching<br>
â€¢ E-Commerce (klein/mittel)<br>
â€¢ Startups<br><br>

Willst du ein individuelles Angebot fÃ¼r dein Projekt?`);
...
âœ… Dein Projekt genau zu verstehen<br>
âœ… Die beste LÃ¶sung zu finden<br>
âœ… Ein konkretes Angebot zu machen<br><br>

Gib mir deine E-Mail und Vorname1/Vorname2 melden sich in 24h mit TerminvorschlÃ¤gen.`);
...
Du siehst frÃ¼h ein klares Layout und gibst Feedback.<br><br>
...
<strong>Dauer:</strong> Landingpage 2-4 Wochen, grÃ¶ÃŸere Projekte 4-8 Wochen.<br><br>

Willst du ein Angebot fÃ¼r dein Projekt?`);
  ...
  saveConversationToStorage() {
    localStorage.setItem('MASESites_chat_history', JSON.stringify(this.conversationHistory));
    localStorage.setItem('MASESites_lead_data', JSON.stringify(this.leadData));
  }

  loadConversationFromStorage() {
    const history = localStorage.getItem('MASESites_chat_history');
    const leadData = localStorage.getItem('MASESites_lead_data');

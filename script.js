...
// ============================================
// LIGHTHOUSE-DASHBOARD (Code-Proof Sektion)
// ============================================
(function () {
  const dash = document.getElementById('lhd');
  if (!dash) return;

  const reduceMotion = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
  const finePointer = window.matchMedia('(hover: hover) and (pointer: fine)').matches;
  const counters = Array.from(dash.querySelectorAll('[data-lhd-count]'));

  // — Eintritts-Animation: Ring füllt sich, Zahlen zählen hoch —
  if (!reduceMotion && 'IntersectionObserver' in window) {
    dash.classList.add('lhd-prep');
    ...
    const obs = new IntersectionObserver((entries, o) => {
      entries.forEach(e => {
        ...
        dash.classList.add('lhd-on');
        counters.forEach((el, i) => {
          const target = parseInt(el.getAttribute('data-lhd-count'), 10) || 0;
          setTimeout(() => countUp(el, target, 1300), 250 + i * 120);
        });
        o.disconnect();
      });
    }, { threshold: 0.35 });
  ...
  // — Parallax + Card-Tilt: nur Desktop mit Maus, nie bei Reduced Motion —
  ...
  dash.addEventListener('mousemove', e => {
    ...
    raf = requestAnimationFrame(() => {
      ...
      dash.style.setProperty('--ppx', (-x).toFixed(3));
      dash.style.setProperty('--ppy', (-y).toFixed(3));
    });
  });
  dash.addEventListener('mouseleave', () => {
    dash.style.setProperty('--ppx', '0');
    dash.style.setProperty('--ppy', '0');
  });

  dash.querySelectorAll('.lhd-card').forEach(card => {

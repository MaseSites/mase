...
CREATE TABLE IF NOT EXISTS mase_leads (
  ...
  category         text DEFAULT 'question'
                   CHECK (category IN ('new_lead','question','meeting_request','website_request','support','other')),
  message          text,
  source           text DEFAULT 'chatbot',
  status           text DEFAULT 'new'
                   CHECK (status IN ('new','in_progress','contacted','done','qualified','closed','lost')),
  notes            text
);
...
-- Safe to run even if column already exists (DO block handles)
-- ============================================================
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name='mase_leads' AND column_name='category'
  ) THEN
    ALTER TABLE mase_leads
      ADD COLUMN category text DEFAULT 'question'
        CHECK (category IN ('new_lead','question','meeting_request','website_request','support','other'));
  END IF;
END
$$;

-- Migration: expand status constraint on existing databases
-- Drop old constraint, add new one that includes in_progress/done
DO $$
BEGIN
  ALTER TABLE mase_leads DROP CONSTRAINT IF EXISTS mase_leads_status_check;
  ALTER TABLE mase_leads
    ADD CONSTRAINT mase_leads_status_check
      CHECK (status IN ('new','in_progress','contacted','done','qualified','closed','lost'));
...
CREATE INDEX IF NOT EXISTS idx_leads_category  ON mase_leads (category);
